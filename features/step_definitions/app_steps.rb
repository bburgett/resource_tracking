...
When /^I fill in the percentage for "Human Resources For Health" with "([^"]*)"$/ do |amount|
   steps %Q{ When I fill in "activity_budget_amounts_1_percentage" with "#{amount}"}
end

Then /^the percentage for "Human Resources For Health" field should equal "([^"]*)"$/ do |amount|
  steps %Q{ Then the "activity_budget_amounts_1_percentage" field should equal "#{amount}"}
end
